 ...
 **/
if (
  document.getElementById('DataObjectOneFieldUpdateUL') !== null &&
  typeof document.getElementById('DataObjectOneFieldUpdateUL') !== 'undefined'
) {
  ;(function ($) {
  ...
  var DataObjectOneFieldUpdate = {
    ...
    inputSelector:
      '#DataObjectOneFieldUpdateUL input.updateField, #DataObjectOneFieldUpdateUL textarea.updateField, #DataObjectOneFieldUpdateUL select.updateField',
    ...
    dirtyfixes: function () {
      var elementType = jQuery(
        '#DataObjectOneFieldUpdateUL li span .updateField'
      )
        .first()
        .prop('nodeName')
        .toLowerCase()
      if (elementType == 'select') {
        var selectID = jQuery(
          ...
          .first()
          .attr('id')
        jQuery('select#' + selectID)
          .first()
          .clone()
          .attr({ name: 'ApplyToAll', id: 'ApplyToAll' })
          .insertAfter('#ApplyToAllButton')
      } else {
        var inputType = jQuery('#DataObjectOneFieldUpdateUL li input')
          .first()
          .attr('type')
          .toLowerCase()
        var inputValue = jQuery('#DataObjectOneFieldUpdateUL li input')
          .first()
          .val()
        jQuery("<input type='" + inputType + "' />")
          .attr({ value: inputValue, name: 'ApplyToAll', id: 'ApplyToAll' })
          .insertAfter('#ApplyToAllButton')
      }
    },
    ...
    init: function () {
      this.dirtyfixes()
      DataObjectOneFieldUpdate.fieldName = jQuery(
        DataObjectOneFieldUpdate.fieldNameSelector
      ).val()
      DataObjectOneFieldUpdate.tableName = jQuery(
        DataObjectOneFieldUpdate.tableNameSelector
      ).val()
    ...
    setupChangeListener: function () {
      jQuery(DataObjectOneFieldUpdate.inputSelector)
        .change(function () {
          var el = this
          ...
          if (parseInt(id)) {
            if (DataObjectOneFieldUpdate.fieldName) {
              ...
              jQuery(DataObjectOneFieldUpdate.feedbackSelector).html(
                DataObjectOneFieldUpdate.loadingText
              )
              DataObjectOneFieldUpdate.updateServer(ids, value, el)
            } else {
              jQuery(DataObjectOneFieldUpdate.feedbackSelector).html(
                'ERROR: could not find field to update'
              )
            }
          } else {
            jQuery(DataObjectOneFieldUpdate.feedbackSelector).html(
              'ERROR: could not find record to update'
            )
          }
        })
        .closest('li.fieldHolder')
        .addClass('readyForAction')
    },

    setupFilter: function () {
      jQuery('#TextMatchFilter').on('input', function (event) {
        event.preventDefault()
        var filterValue = jQuery('#TextMatchFilter').val().toLowerCase()
        jQuery('#DataObjectOneFieldUpdateUL li label').each(function (
          index,
          value
        ) {
          var match = true
    ...
    setupApplyAll: function () {
      jQuery('#ApplyToAllButton').on('click', function (event) {
        event.preventDefault()
        var canDo = confirm(
        ...
        if (canDo == true) {
          ...
          if (elementType == 'select') {
            ...
            jQuery(inputSelector).each(function (index, el) {
              var currentInput = jQuery(el)
              currentInput.val(applyToAllValue)
              var idAndValue =
                DataObjectOneFieldUpdate.retrieveDetailsFromInput(currentInput)
          ...
          DataObjectOneFieldUpdate.updateServer(
            ids,
            applyToAllValue,
            inputSelector
          )
